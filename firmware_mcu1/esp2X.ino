...
#include <esp_now.h>
#include <WiFi.h>             // Changed to standard Arduino WiFi for coexistence
...
// Initialize the LCD (0x27 is standard, change to 0x3F if it doesn't show text)
...
// Use volatile for variables modified in interrupts/callbacks
...
// NEW: Helper function to refresh only the score value on the LCD
void updateLCDScore() {
    lcd.setCursor(7, 0);   // Move cursor to just after "Score: "
    lcd.print("         "); // Clear previous numbers
    lcd.setCursor(7, 0);   // Reset cursor position
    lcd.print(score);      // Print updated score
}

// Updated Callback for ESP32 Core 3.0.0+
...
void setup() 
{
    ...
    // Initialize the LCD once here
    lcd.init();
    lcd.backlight();
    lcd.setCursor(0, 0);
    lcd.print("Score: 0"); // Base text drawn once at startup
    
    // 1. Initialize Bluetooth First
    ...
    // 2. Initialize Wi-Fi safely using the Arduino Core
    WiFi.mode(WIFI_STA);
    WiFi.disconnect(); 
    
    // 3. Coexistence Magic Fix
...
void loop() 
{
    // 1. Process incoming Bluetooth commands from the App
    if (SerialBT.available() >= 2) 
    {
        ...
        if (is_correct) 
        {
            if (cmd == 'W') 
            {
                ...
                score++;
                updateLCDScore(); // Updated instantly here
    ...
    // 2. Process Position changes triggered by ESP-NOW timestamps
